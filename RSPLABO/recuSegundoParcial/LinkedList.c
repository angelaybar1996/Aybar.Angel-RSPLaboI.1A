...
/** \brief Recibe una lista y una funcion.Se iteran todos los elementos de la lista
 y se los pasa a la funcion que recibira el elemento y le calculará el campo tiempo.
 *
 * \param element void*
 * \return El retorno de pFunc se agregara a la lista resultado,esta nueva lista sera devuelta por ll_map
 *
 */
 /*
    ...
    if(this!=NULL)
    {
        ...
        if(nuevoLinkedList!=NULL && pFunc!=NULL)
        {
            for(int i=0;i<ll_len(this);i++)
            {
                ...
                if(pFunc(pAuxiliar)==1)
                {
                    ll_add(nuevoLinkedList,pAuxiliar);
                }
            }
            pAuxiliar=NULL;
        }
